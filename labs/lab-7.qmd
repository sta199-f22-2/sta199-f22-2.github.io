...
Close-issue question (insert here)
...
a Specify a logistic regression model using "glm" as the engine, that predicts `advfront` by `edu`. Name this specification gss16_spec. Report the tidy output below.


b Write out the estimated model in proper notation. 

c Using your estimated model, predict the probability of agreeing with the following statement: Even if it brings no immediate benefits, scientific research that advances the frontiers of knowledge is necessary and should be supported by the federal governmen (Agree in advfront) if you have an education of 7 years.
...
b Using sound statistical reasoning, make an argument for the "better" model between the one fit in exercise 2, and exercise 3. 
...
::: callout-important
Pick another member of the team write the answer to Exercise 3=4. All others should contribute to the discussion but only one person should type up the answer, render the document, commit, and push to GitHub. All others should **not** touch the document.
:::
