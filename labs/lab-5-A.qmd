...
(a) *Suggested Answer* - AWV. Example: I think about the fundementals of probability when I cross the street. The probability of being hit by a car is much lower when I look both ways / there are no cars in the area than when there are cars around and I am more careless. 
...
data <- read_csv("labs/data/education-disease.csv")
...
*Suggested Answer* Many ways to get to the same answer. There are 4 distinct levels of education and 2 distinct levels of disease.
...
(b) Use `count` and `pivot-wider` to create a 4 x 3 data table below. 

```{r}

data |>
  count(Education, Disease) |>
...
*Suggested Answer* (Can also just put the total for the denominator)

```{r}
(323 + 88 + 147 + 70) / (70 + 147 + 88 + 323 + 403 + 1106 + 601 + 1397)
```
...
```{r}
(1106) / (70 + 147 + 88 + 323 + 403 + 1106 + 601 + 1397)
...
*Suggested Answer* There seems to be a moderate positive relationship between Temperature and Bikes Rented. As the temperature increases, so do the number of bikes rented. 
```{r}
bike |>
...
*Suggested Answer* AWV. Key points include seeing an overall relationship change when looking at sub-relationships. 

(d) Compare and contrast your findings from part (a) and part (b). What's different?

*Suggested Answer* The relationship between temperature and bikes rented changed between parts a and b. In part a, we observed an overall positive relationship, while now we observe four negative relatinoships when looking at temperature and bikes rented by month. 
...
*Suggested Answer* AWV. Need to relate answer to both explanatory and reponse variable. Example: A potential confounding variable could be *time of day*. Time of day could have a relationship with the number of bikes rented, such as more people tend to rent bikes in the morning than in the afternoon, and also has a relationship with Temperature (i.e. It is often colder in the morning and nights than in the afternoon / evening). Time of day may be influencing the relationship we observe in these plots. 
