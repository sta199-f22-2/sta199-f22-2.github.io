...
(a) Using `Selector Gadget`, scrape the `tomato_score` and `audience_score` and report the lengths of these vectors (using the `length()`) function. *Hint:* Their lengths should be equal.
...
halloween <- tibble(title_year = titles_years) |>
  separate(titles, into = c("title", "year"), sep= "\\(" )
```
...
(d) Create an appropriate plot the assess the relationship between a movie's audience score and their tomatometer score. In 2-3 sentences, comment on the relationship. Your plot should include appropriate labels.
...
(b) Plotting the data is a useful first step because it helps us quickly visualize trends, identify strong associations, and develop research questions. Create an appropriate plot displaying the relationship between`habit` and `weight`. In 2-3 sentences, discuss the relationships observed.

(c) Now, fit a linear model that investigates the relationship between `weight` and `births14_habigtgiven`. Provide the tidy summary output below.
...
-   Exercise 1: ___ points
-   Exercise 2: ___ points
-   Exercise 3: ___ points
-   Exercise 4: ___ points
-   Exercise 5: ___ points
-   Exercise 6: ___ points
