...
## Exercise 1 - Data Scraping (Need to Write)
...
Clean the data set and plot the relationship between tomato and popcorn ratings
...
titles <- titles[c(FALSE, TRUE)]


reviews <- page |>
  html_nodes(".franchise-media-list__tomatometer strong") |>
  html_text2()

popcorn <- page |>
  html_nodes(".franchise-media-list__audiences strong") |>
  html_text2()

amazon <- tibble(
  title = titles,
  review = reviews,
  country_date = popcorn
)
```
...
## Exercise 2 - The Work of Bob Ross
...
## Exercise 3 - America's Neighborhood Pollster 
...
(e) Create an appropriate visualization to compare the the relationship between `Age` and `Mood`. Hint: You may have to re manipulate the data into a more usable form.

```{r}

# segmented bar-graph plot here
...
(a) Recreate this graph: 

```{r}

candy |>
  mutate(chocolate = as.factor(chocolate)) |>
  ggplot(aes(x = sugarpercent, y = pricepercent , color = chocolate)) + 
  geom_point(shape = 2) +
  geom_smooth(method = "lm" , se = F, linetype = "dashed") +
  labs(
    title = "Sugar % by Price",
    subtitle = "and if chocolate is present",
    color = "Chocolate" , 
    y = "Price Percent ",
    x = "Sugar Percent"
  )
...
trick_or_treat <- function(people){
  candy <- c('chocolate', 'starbursts')
  results <- sample(candy, size = people, replace = TRUE)
